...
class Orchestrator:
    ...
    def _synthesize(self, query: str, mode: str, tool_outputs: Dict[str, str]) -> str:
        system = (
            ...
            "要求：\n"
            "1. **智能降级策略**（至关重要）：\n"
            "   - 如果数据库查询结果包含错误信息（如 'error'）、为空、或只是无法连接等技术报错，**请立即放弃使用数据库结果**。\n"
            "   - 在这种情况下，**完全信赖并使用 Web 搜索结果**来回答用户问题。\n"
            "   - **不要**在回答中反复强调“数据库报错”或“无法查询”，而是直接给出基于 Web 信息的最佳回答，表现得像一个自信的助手。\n"
            "2. **数据优先权**：\n"
            "   - 只有当数据库查询**成功**且返回了**有效数据**时，才优先以 DB 结果为准。\n"
            "3. **内容规范**：\n"
            "   - DB 里没有的字段不要臆造。\n"
            "   - 回复末尾需标明主要来源（例如：'来源：Web 搜索' 或 '来源：数据库统计'）。\n"
        )
        if mode == "report":
            system += (
                "输出 Markdown 报告，包含：\n"
