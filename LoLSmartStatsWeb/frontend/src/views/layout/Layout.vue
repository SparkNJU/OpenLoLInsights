  ...
  <div class="flex h-screen w-full bg-gray-50">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10">
      <div class="h-16 flex items-center px-6 border-b border-gray-100">
        <div class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center mr-3 text-white font-bold text-xl">L</div>
      ...
      <div class="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
        ...
        <template v-if="isLoggedIn">
          <div v-for="session in sessions" :key="session.id">
            ...
            >
              <el-icon :size="20" class="mr-3"><ChatDotRound /></el-icon>
              {{ session.name }}
            </router-link>
          </div>
        </template>
        ...
        >
          <el-icon :size="20" class="mr-3"><DataAnalysis /></el-icon>
          赛事数据中心
        </router-link>
        ...
        >
          <el-icon :size="20" class="mr-3"><PieChart /></el-icon>
          统计图表
        </router-link>
      </div>
